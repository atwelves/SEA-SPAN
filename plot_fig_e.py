...
# time series
ds   = xr.open_dataset('time_series/BO_TS_TG_{}_processed_UNFILTERED.nc'.format(station))
print(ds)
obs  = ds.ssh_obs
...
obs_spectrum = np.zeros((31,17))
mod_spectrum = np.zeros((31,17))

# transforms
meso           = xr.open_dataarray('{}/{}_obs_meso_{}.nc'.format(station,station,month)).values
# synoptic
syno           = xr.open_dataarray('{}/{}_obs_syno_{}.nc'.format(station,station,month)).values
# concatenate
obs_spectrum[1:30,0:8]  = meso
obs_spectrum[1:30,8:16] = syno

# mesoscale
meso           = xr.open_dataarray('{}/{}_mod_meso_{}.nc'.format(station,station,month)).values
# synoptic
syno           = xr.open_dataarray('{}/{}_mod_syno_{}.nc'.format(station,station,month)).values
# concatenate
mod_spectrum[1:30,0:8]  = meso
mod_spectrum[1:30,8:16] = syno
...
plt.contourf(np.linspace(0.5,30.5,31),np.linspace(0,16,17),10000*np.transpose(dif_spectrum),levels=[-300,-200,-100,0,100,200,300],colors=custom,extend='both')
plt.grid()
...
plt.plot(np.linspace(1,30,29*24),10*obs[123*24:152*24]+19,linewidth=3,color='k',label='obs')
plt.plot(np.linspace(1,30,29*24),10*mod[123*24:152*24]+19,linewidth=3,color='k',linestyle='--',label='mod')
...
plt.ylim(-13,33)
plt.savefig('fig_e/{}_transform_zoom_{}'.format(station,month))
