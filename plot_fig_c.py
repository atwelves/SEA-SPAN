...
# now plot anomaly (UNFILTERED)
    ...
    plt.scatter(mes_dif,syn_dif,s=250,label='{}'.format(st),c=color_range,cmap='twilight',edgecolors='k')
    plt.scatter(mes_dif[144],syn_dif[144],s=1500,label='{}'.format(st),facecolors='none',edgecolors='k',linewidths=3)
    plt.colorbar()

    plt.xticks(fontsize=30,rotation=45)
    plt.yticks(fontsize=30,rotation=45)
    plt.ylabel('$\Delta W²_{syn}$ (m²)',fontsize=45)
    plt.xlabel('$\Delta W²_{meso}$ (m²)',fontsize=45)
    plt.title('({}) {}'.format(letter[i],st),fontsize=45)
    plt.grid()
    plt.locator_params(axis='y', nbins=7)
    plt.locator_params(axis='x', nbins=7)
    plt.tight_layout()
    plt.savefig('fig_c/ratio_amom_seasonal_{}.png'.format(st))
